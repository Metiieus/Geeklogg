...
import { useMedias, useAddMedia, useUpdateMedia, useDeleteMedia } from "../../hooks/queries";
...
const ProLibrary: React.FC = () => {
  ...
  const [customFeatured, setCustomFeatured] = useState<MediaItem[]>([]);
  const [customPopular, setCustomPopular] = useState<MediaItem[]>([]);
  ...
  // Carregar destaques e melhores salvos ou defaults
  useEffect(() => {
    ...
    if (savedFeaturedIds) {
      try {
        ...
        if (savedItems.length > 0) {
          ...
          setCustomFeatured(mediaItems.slice(0, 4));
        }
      } catch (error) {
    ...
    // Logic: If no saved popular, derive from top rated
    // Note: Library didn't strictly save "popular" IDs to local storage in previous code, 
    // it used `topRated` prop. But `handleSavePopular` calls `setCustomPopular`.
    // It seems `handleSavePopular` didn't save to localStorage in the previous code?
    // Let's check: "const handleSavePopular = ... setCustomPopular(items); showSuccess..." 
    // It did NOT save to local storage. So it was transient or relied on prop.
    // We should default to top rated calculation.
    if (customPopular.length === 0) {
      const top = [...mediaItems]
        .filter((m) => m.rating !== undefined)
        .sort((a, b) => (b.rating ?? 0) - (a.rating ?? 0))
        .slice(0, 6);
      setCustomPopular(top);
    }

    // Carregar melhores mídias (Podium)
  ...
  const handleConfirmAdd = useCallback(async (media: ExternalMediaResult) => {
    ...
    try {
      ...
      // Normalizar tipo para MediaType válido
      const normalizeType = (type: string): any => {
        const t = type.toLowerCase();
        if (t.includes("book") || t.includes("livro")) return "book";
        if (t.includes("game") || t.includes("jogo")) return "game";
        if (t.includes("movie") || t.includes("filme")) return "movie";
        if (t.includes("tv") || t.includes("series") || t.includes("série")) return "tv";
  ...
  }, [addMediaMutation, showSuccess, showError]);
  ...
  const handleUpdateMedia = useCallback(async (id: string, updates: Partial<MediaItem>) => {
    try {
      await updateMediaMutation.mutateAsync({ id, updates });
  ...
  }, [updateMediaMutation, showSuccess, showError]);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await deleteMediaMutation.mutateAsync(deleteConfirmItem.id);
  ...
  const handleToggleFavorite = useCallback(async (item: MediaItem) => {
    try {
      ...
      await updateMediaMutation.mutateAsync({ id: item.id, updates: { isFavorite: newFavoriteStatus } });
      showSuccess(
