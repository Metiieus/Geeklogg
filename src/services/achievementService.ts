...
export async function getUserAchievements(): Promise<UserAchievement[]> {
  const uid = getUserId();
  const snap = await database.getCollection<UserAchievement>(['users', uid, 'achievements']);
...
export function getAchievementsWithProgress(userAchievements: UserAchievement[]): AchievementNode[] {
  ...
  return ACHIEVEMENTS_DATA.map(achievement => {
    ...
    return {
      ...
      unlocked: unlocked || (canUnlock && checkAchievementCondition(achievement.id))
    };
  });
}

export function getAchievementProgress(userAchievements: UserAchievement[]): AchievementProgress {
  const totalAvailable = ACHIEVEMENTS_DATA.length;
  const totalUnlocked = userAchievements.length;
  
  const byCategory = ACHIEVEMENTS_DATA.reduce((acc, achievement) => {
  ...
  for (const achievement of ACHIEVEMENTS_DATA) {
    ...
    switch (achievement.id) {
      ...
      case 'completou_primeiro_game':
        shouldUnlock = mediaItems.some(item => 
          item.type === 'games' && item.status === 'completed'
        );
        break;
        
      case 'viciado_em_horas':
        shouldUnlock = mediaItems.some(item => 
          item.type === 'games' && (item.hoursSpent || 0) >= 100
        );
        break;
      ...
      case 'devorador_de_livros':
        shouldUnlock = mediaItems.filter(item => 
          item.type === 'books' && item.status === 'completed'
        ).length >= 10;
        break;
        
      case 'primeiro_filme':
        shouldUnlock = mediaItems.some(item => 
          item.type === 'movies' || item.type === 'series'
        );
        break;
      ...
      case 'critico_experiente':
        shouldUnlock = reviews.filter(review => review.content.length >= 100).length >= 5;
        break;
