    ...
    try {
      ...
      if (db && !firebaseOffline) {
        await withRetry(async () => {
          ...
          return testDoc;
        }, 1, 500); // Single retry with short delay for quick test
        databaseStatus = true;
      }
    } catch (error: any) {
...
export const logConnectivityStatus = async () => {
  ...
  console.log(`  🌐 Network: ${result.networkStatus ? '✅ Online' : '❌ Offline'}`);
  console.log(`  🔥 Firebase: ${result.firebaseStatus ? '✅ Connected' : '❌ Disconnected'}`);
  console.log(`  🔐 Auth: ${result.authStatus ? '✅ Available' : '❌ Unavailable'}`);
  console.log(`  💾 Database: ${result.databaseStatus ? '✅ Accessible' : '❌ Inaccessible'}`);
